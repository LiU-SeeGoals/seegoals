...
case $choice in
  1)
    ...
    docker compose -p config1 up -d
    ;;
  2)
    echo "Starting with local controller and GHCR game viewer..."
    docker compose -p config2 -f docker-compose.yml -f docker-compose.local-controller.yml up --build -d
    echo -e "${CYAN}Entering controller container...${NC}"
    docker compose -p config2 exec controller sh
    ;;
  3)
    echo "Starting with local game viewer and GHCR controller..."
    docker compose -p config3 -f docker-compose.yml -f docker-compose.local-gameviewer.yml up --build -d
    echo -e "${CYAN}Entering game-viewer container...${NC}"
    docker compose -p config3 exec game-viewer bash
    ;;
  4)
    echo "Starting with local controller and local game viewer..."
    docker compose -p config4 -f docker-compose.yml -f docker-compose.local-controller.yml -f docker-compose.local-gameviewer.yml up --build -d --force-recreate
    echo -e "${GREEN}Both controller and game-viewer are local. Not entering any container.${NC}"
    ;;
  5)
    echo "Starting setup for real cameras"
    docker compose -p config5 -f docker-compose.yml -f docker-compose.real-robots.yml -f docker-compose.local-controller.yml up --build -d --force-recreate
